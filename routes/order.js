...
orderRoute.get("/", authAdmin, async (req, res, next) => {
  try {
...
orderRoute.get("/myOrder", authenticate, async (req, res, next) => {
...
orderRoute.post("/", authenticate, async (req, res, next) => {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (oldQuantity - products[i].quantity < 0) {
        return res.send(
          `${products[i].productId} is sold out, we have only ${oldQuantity} items left in the stock`
        );
      }
      await Product.updateOne(
        { _id: id },
        { quantity: oldQuantity - products[i].quantity }
      );
    }
...
orderRoute.patch("/:id", authenticate, authAdmin, async (req, res, next) => {
  ...
  try {
    ...
    res.status(404).json({
      message: "Order status has been edited",
      error: err,
    });
  }
});
...
orderRoute.delete("/:id", authenticate, authAdmin, async (req, res, next) => {
  const id = req.params.id;
...
orderRoute.delete("/", checkAuth, async (req, res, next) => {
  const id = req.params.id;
